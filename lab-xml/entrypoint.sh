...
echo "Starting Lab XML Injection..."

if [ ! -d "/var/lib/mysql/labxml" ]; then
    ...
    mysql -u root <<-EOSQL
        CREATE DATABASE IF NOT EXISTS labxml CHARACTER SET utf8mb4;
        CREATE USER IF NOT EXISTS 'labuser'@'localhost' IDENTIFIED BY 'labpass123';
        GRANT ALL PRIVILEGES ON labxml.* TO 'labuser'@'localhost';
        FLUSH PRIVILEGES;
EOSQL

    mysql -u root labxml < /docker-entrypoint-initdb.sql
    echo "Database ready!"
