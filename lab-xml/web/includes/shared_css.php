<?php /* shared_css.php — include di setiap halaman */ ?>
<style>
...
/* NAV */
.nav{position:sticky;top:0;z-index:100;background:rgba(8,11,19,.97);backdrop-filter:blur(16px);border-bottom:1px solid var(--bd);height:60px;display:flex;align-items:center;padding:0 40px;gap:32px}
...
.bc-sep{color:var(--t3)}
.phdr h1{font-size:1.5rem;font-weight:800;letter-spacing:-.02em;margin-bottom:8px;display:flex;align-items:center;gap:12px;flex-wrap:wrap}
.phdr-desc{font-size:.87rem;color:var(--t2);line-height:1.75;max-width:680px}

/* BADGE/TAG */
.tag{font-size:.6rem;font-weight:700;letter-spacing:.1em;padding:3px 10px;border-radius:20px;font-family:var(--mono);border:1px solid;white-space:nowrap}
...
.kw{color:var(--red)}.val{color:var(--blue)}.str{color:var(--orange)}.cm{color:var(--t3)}.tag-xml{color:var(--purple)}.attr{color:var(--green)}
...
.tbl td{padding:9px 14px;border-bottom:1px solid var(--bd);color:var(--t2);font-family:var(--mono);vertical-align:top}
...
/* SPOILER */
.spoiler{margin-bottom:14px}
.spoiler summary{cursor:pointer;font-size:.72rem;color:var(--t3);font-family:var(--mono);list-style:none;display:inline-flex;align-items:center;gap:6px;user-select:none;padding:6px 0}
